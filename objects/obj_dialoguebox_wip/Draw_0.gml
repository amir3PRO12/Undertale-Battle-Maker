		...
		if page < page_number-1{
			...
			if choice_page == page {
				...
				if string_starts_with(text[page],"$") {
					setup = false;
					var _arr = string_split(text[page],"$",true,2)
					if array_length(_arr) == 1 {
						array_push(_arr,0)	
					}
					text = library[$ _arr[0]][real(_arr[1])]
...
if choice_page == page {
	...
	for (var _choice = 0; _choice <  _choices_num; _choice++) {
		draw_text_ext(dialogue_box_x + text_x_offset[page] + border_x + (choice_width)*(_choice),
					dialogue_box_y + border_y + string_height(text[page]), 
